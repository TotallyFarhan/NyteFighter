...
class Rocket {
    ...
    draw() {
        ...
        ctx.fillStyle = 'white';
        ctx.font = '30px Arial';
        ctx.fillText(this.ammo, this.x + this.width / 3, this.y + this.height / 1/2)
    }
    //rocket update func
    update() {
        this.draw();
...
//rocket declaration
let rocket = new Rocket(playerX, playerY, playerWidth, playerHeight, playerColor, 0, playerAmmo);
...
const spawnBoss = () => {
    setInterval(() => {
        if (!bossStatus && currentlyPlaying && score >= 100) {
            bosses.push(new Boss(canvas.width + 300, canvas.height/2, 300, 3, 1500, 5000));
            bossStatus = true;
            bossSpawned = true;
        }
    }, 1)
...
const spawnFastEnemies = () => {
    setInterval(() => {
        ...
        if (score >= 75 && currentlyPlaying && !bossSpawned) {
            enemies.push(new Enemy(fastenemyX, fastenemyY, fastenemyRadius, fastenemyColor, fastenemySpeed, fastenemyHealth))
        }
    }, 5000)
}
...
const spawnPowerups = () => {
    setInterval(() => {
        ...
        if (randomNum === 1 && currentlyPlaying) {
            powerups.push(new Powerup(livespowerupX, livespowerupY, livespowerupRadius, livespowerupColor, livespowerupSpeed, livespowerupAction, livespowerupImageUrl));
        }
    ...
    setInterval(() => {
        ...
        if (randomNum2 === 3 && currentlyPlaying) {
            powerups.push(new Powerup(immunpowerupX, immunpowerupY, immunpowerupRadius, immunpowerupColor, immunpowerupSpeed, immunpowerupAction, immunpowerupImageUrl));
        }
    }, 5000)
    setInterval(() => {
        ...
        if (randomNum3 === 2 && currentlyPlaying) {
            powerups.push(new Powerup(speedpowerupX, speedpowerupY, speedpowerupRadius, speedpowerupColor, speedpowerupSpeed, speedpowerupAction, speedpowerupImageUrl));
        }
    }, 7000)
    setInterval(() => {
        ...
        if (randomNum4 == 3 && currentlyPlaying) {
            powerups.push(new Powerup(wallpowerupX, wallpowerupY, wallpowerupRadius, wallpowerupColor, wallpowerupSpeed, wallpowerupAction, wallpowerupImageUrl))
        }
    }, 10000)
    setInterval(() => {
        ...
        if (randomNum5 == 3 && currentlyPlaying) {
            powerups.push(new Powerup(multiplierpowerupX, multiplierpowerupY, multiplierpowerupRadius, multiplierpowerupColor, multiplierpowerupSpeed, multiplierpowerupAction, multiplierpowerupImageUrl))
        }
    }, 7000)
    setInterval(() => {
